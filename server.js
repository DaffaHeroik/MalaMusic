...
app.all('/api/users', require('./api/users.js'));
app.all('/api/blend', require('./api/blend.js'));
...
function isAllowedAudioUrl(value, baseUrl) {
    try {
        const parsed = new URL(String(value || ''), baseUrl);
        const hostName = String(parsed.hostname || '').toLowerCase();
        const allowedMediaHost = hostName === 'googlevideo.com' || hostName.endsWith('.googlevideo.com') || hostName === 'youtube.com' || hostName.endsWith('.youtube.com') || hostName === 'youtu.be' || hostName.endsWith('.youtu.be') || /^cdn\d+\.savetube\.vip$/.test(hostName) || hostName === 'youtubedl.siputzx.my.id';
