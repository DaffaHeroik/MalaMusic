...
function headers() { return { 'Content-Type': 'application/json', 'x-malamusic-secret': process.env.MALAMUSIC_INTERNAL_SECRET || '' }; }
...
module.exports = async function stats(req, res) {
    try {
        ...
        if (action === 'leaderboard') return res.status(200).json(await call('/leaderboard', { method: 'GET' }));
        if (action === 'public-playlist') return res.status(200).json(await call('/playlist/' + encodeURIComponent(String((req.query && req.query.id) || '')), { method: 'GET' }));
