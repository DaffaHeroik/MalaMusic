...
module.exports = async function blend(req, res) {
    ...
    try {
        ...
        if (action === 'state' && method === 'GET') return res.json({ status: true, room: publicRoom(room, user) });
        ...
        if (action === 'invite' && method === 'POST') {
            if (room.ownerUid !== user.uid) return responseError(res, 403, 'Hanya pembuat Blend yang dapat mengundang anggota.'); const inviteUid = text(body.partnerUid, '', 180); const invited = await account(db, inviteUid); if (!invited) return responseError(res, 404, 'Profil pengguna tidak tersedia.'); const update = {}; update[`members/${inviteUid}`] = { ...invited, role: 'member', status: 'pending', joinedAt: 0 }; update.updatedAt = Date.now(); await ref.update(update); room.members[inviteUid] = update[`members/${inviteUid}`]; room.updatedAt = update.updatedAt; return res.json({ status: true, room: publicRoom(room, user), inviteCode: id });
        }
