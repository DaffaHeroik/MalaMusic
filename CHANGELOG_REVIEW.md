...
## Audio resolver latency and transient failure fix — 2026-08-18

A production Frontend Police replay reproduced the audio preparation failure on `Sedia Aku Sebelum Hujan` (`MODbh7nKWmQ`). The browser showed four `/api/ytplay` requests; two current-load requests were aborted at approximately 12 seconds with no response body, while the backend could require roughly 11–12 seconds during an upstream cold/overloaded path. A direct resolver check also returned HTTP 503 for another track before succeeding on later attempts. The frontend resolver timeout was therefore too close to the observed upstream latency, and resolver errors were not explicitly checked with `response.ok`.

The player now uses `AUDIO_RESOLVE_TIMEOUT_MS = 25000`, validates non-2xx resolver responses, and performs one controlled retry for network/5xx/invalid-media failures. The retry is bounded and does not run after the AbortController has been cancelled. Assets and Service Worker cache were bumped from v86 to v87. Validation includes `node --check public/player.js public/sw.js`, `npm run test:playback-race`, and `git diff --check`; production replay and Vercel deployment verification remain required.
...
## Endpoint route hygiene and image proxy — 2026-08-18

A second real production probe found `/api/proxy-image` was not implemented on the Vercel/Express path even though the frontend calls it; production returned the SPA HTML shell with HTTP 200 instead of an image. The same fallback masked `/api/index`, `/api/firebase-admin`, and arbitrary `/api/unknown-route` as HTML 200 responses. A same-origin image proxy was added with HTTPS CDN allowlist, bounded redirect, image content-type validation, 8 MB limit, and timeout. Unknown `/api/*` paths now return JSON 404 before the SPA fallback. Production retest also showed `ytplay` can still return transient 503 from the external extraction provider; this is an upstream availability issue, not an endpoint contract mismatch.
