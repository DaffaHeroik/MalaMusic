var Library={
    ...
    share(id){
        ...
        var text = 'Dengarkan playlist "' + pl.name + '" (' + pl.songs.length + ' lagu) di MalaMusic!';
        if (navigator.share) {
            navigator.share({ title: pl.name + ' - MalaMusic', text: text, url: url }).catch(function(){});
        } else {
        ...
        var html=`
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="library-content" onscroll="Library.handleScroll()">
                ...
                <div class="px-6 mb-6 flex flex-wrap items-center gap-3">
                    ${pl.songs.length>0?`<button onclick="Library.playSong('${id}',0)" class="bg-white hover:bg-gray-200 text-black w-14 h-14 rounded-full flex items-center justify-center active:scale-95 transition-all shadow-white/20"><i data-lucide="play" class="w-7 h-7 fill-current ml-1"></i></button><button onclick="Library.shufflePlaylist('${id}')" class="text-white/70 hover:text-white p-3 rounded-full active:scale-95 bg-white/5 transition-all" title="Acak Urutan (Shuffle)"><i data-lucide="shuffle" class="w-6 h-6"></i></button><button onclick="downloadPlaylistOffline('${id}')" class="flex items-center gap-2 rounded-full px-4 py-3 bg-cyan-500/15 border border-cyan-400/25 text-cyan-200 text-xs font-bold active:scale-95 transition-all" title="Download seluruh playlist ke Mode Offline" aria-label="Download seluruh playlist ke Mode Offline"><i data-lucide="download-cloud" class="w-4 h-4"></i><span>Download Playlist</span><span class="text-cyan-300/70">${offlineCount}/${pl.songs.length}</span></button>`:''}
                </div>
        `;
