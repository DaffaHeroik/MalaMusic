    ...
    show(loading){
        ...
        if (S.filter === 'songs') {
            ...
            c.innerHTML='<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pb-8">'+S.sr.map(function(user, i){
                return '<div onclick="Search.openUser(\''+esJs(user.id)+'\')" role="button" tabindex="0" class="text-left flex items-center gap-3 p-4 rounded-2xl bg-[#20222c] border border-white/10 shadow-xl hover:bg-[#282b38] active:scale-[.98] transition-all animate-card-up cursor-pointer" style="animation-delay:'+Math.min(i*40, 500)+'ms"><img src="'+safeMediaUrl(user.picture || FI, FI)+'" class="w-14 h-14 rounded-full object-cover border border-white/15 shrink-0" onerror="this.src=\''+FI+'\'" /><span class="min-w-0 flex-1"><strong class="block text-white font-bold truncate">'+es(user.name)+'</strong><span class="block text-xs text-white/50 mt-1">Profil publik · Lihat playlist</span><button type="button" onclick="event.stopPropagation();Blend.openFromUser(\''+esJs(user.id)+'\',\''+esJs(user.name)+'\')" class="mt-2 px-2.5 py-1.5 rounded-lg bg-fuchsia-400/15 border border-fuchsia-300/30 text-fuchsia-100 text-[10px] font-bold">Buat Blend</button></span><i data-lucide="chevron-right" class="w-4 h-4 text-white/40 ml-auto shrink-0"></i></div>';
            }).join('')+'</div>';
            lucide.createIcons();
