    ...
    show() {
        ...
        if (S.hp && S.hp.length > 0) {
            S.hp.slice(0, 8).forEach(function(p, i) {
                ...
                plHtml += '<div onclick="Album.open(&quot;'+esJs(p.id)+'&quot;, &quot;'+esJs(playlistCover)+'&quot;)" class="flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-2xl bg-[#20222c] border border-white/10 shadow-xl group-hover:bg-[#282b38] transition-all flex flex-col"><div class="w-full aspect-square rounded-xl overflow-hidden relative shadow-md mb-2"><img src="'+playlistCover+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=&quot;/logo.png&quot;" /></div><h3 class="font-semibold text-sm text-white truncate px-0.5">'+es(p.title)+'</h3><p class="text-xs text-white/60 truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div></div>';
            });
        } else if (!pls.length) {
