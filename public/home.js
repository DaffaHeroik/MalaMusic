var Home = {
    ...
    render() {
        var recentTracks = typeof getRecentTracks === 'function' ? getRecentTracks() : [];
        var recentHtml = recentTracks.length ? '<div><div class="flex items-center justify-between mb-3"><h2 class="text-lg font-bold text-white tracking-wide flex items-center gap-2.5"><i data-lucide="history" class="w-5 h-5 text-rose-400"></i><span>Recently Played</span></h2><button onclick="App.switch(\'library\')" class="text-xs font-bold text-white/50 hover:text-white">Lihat koleksi</button></div><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2">' + recentTracks.slice(0,6).map(function(t){ var recentCover = safeMediaUrl(t.cover || FI, FI); return '<button onclick="App.autoPlayTrack(\'' + String(t.videoId || t.id).replace(/'/g,'') + '\')" class="w-32 sm:w-40 shrink-0 p-0 m-0 border-0 bg-transparent text-left align-top group"><div class="aspect-square rounded-xl overflow-hidden bg-transparent p-0 m-0 border-0 shadow-none leading-none mb-2"><img src="' + recentCover + '" class="home-artwork-fill w-full h-full block p-0 m-0 border-0 object-cover group-hover:scale-105 transition-transform" onerror="this.src=\'' + FI + '\'" /></div><p class="text-sm font-semibold text-white truncate">' + es(t.title || 'Lagu') + '</p><p class="text-xs text-white/50 truncate">' + es(t.artist || '') + '</p></button>'; }).join('') + '</div></div>' : '';
            ...
            <div class="glass-strong rounded-3xl p-5 border border-white/15 bg-gradient-to-br from-white/10 via-white/5 to-transparent relative overflow-hidden ">
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left relative z-10">
                    ...
                    <div class="flex-1">
                        ...
                        <h2 class="text-2xl font-black text-white">MalaMusic</h2>
                        <p class="text-xs text-[#b3b3b3] mt-1 leading-relaxed">Pengembang & Pembuat MalaMusic. Selamat menikmati streaming musik favorit tanpa gangguan!</p>
    ...
    show() {
        ...
        if ((!S.ht || S.ht.length === 0) && !navigator.onLine) {
            ...
            g.innerHTML = quickSongs.map(function(t, i) {
                ...
                if (isLoad) {
                    ...
                    playIconHtml = '<div class="w-7 h-7 rounded-full btn-chrome flex items-center justify-center shrink-0 ml-auto"><i data-lucide="play" class="w-3.5 h-3.5 fill-current ml-0.5"></i></div>';
                }
                ...
                return '<div onclick="PK(\'home1\','+i+')" class="home-grid-card '+cardBg+' rounded-2xl p-0 min-w-0 overflow-hidden flex items-center gap-1 sm:gap-2 cursor-pointer active:scale-[0.98] transition-all duration-300 group shadow-lg shadow-black/25">'+
                    '<div class="w-14 h-14 sm:w-[72px] sm:h-[72px] shrink-0 self-stretch overflow-hidden rounded-l-2xl bg-black/20"><img src="'+coverUrl+'" class="home-artwork-fill w-full h-full object-cover block" onerror="this.src=\''+FI+'\'" /></div>'+
        ...
        pls.forEach(function(p, i) {
            plHtml += '<div onclick="'+(p.source === 'youtube' ? 'Album.open(\''+esJs(p.externalId || p.id)+'\',\''+esJs(p.image || '')+'\')' : 'Library.open(\''+p.id+'\')')+'" class="relative flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-2xl bg-[#20222c] border border-white/10 shadow-xl group-hover:bg-[#282b38] transition-all flex flex-col"><div class="w-full aspect-square rounded-xl overflow-hidden relative shadow-md mb-2"><img src="'+(p.image||(p.songs.length>0?p.songs[0].cover:FI))+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=\''+FI+'\'" /><button data-saved-playlist-id="'+esJs(p.externalId || p.id)+'" onclick="event.stopPropagation();'+(p.source === 'youtube' ? 'toggleSavedExternalPlaylist(\''+esJs(p.externalId || p.id)+'\',\''+esJs(p.name)+'\',\''+esJs(p.image || '')+'\',\''+esJs(p.creator || '')+'\')' : 'Library.showActions(\''+p.id+'\')')+'" class="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/65 border border-white/20 backdrop-blur-md flex items-center justify-center" title="'+(p.source === 'youtube' ? 'Hapus dari Koleksi' : 'Opsi Playlist')+'"><i data-lucide="'+(p.source === 'youtube' ? 'bookmark-check' : 'more-vertical')+'" class="w-4 h-4 '+(p.source === 'youtube' ? 'text-amber-300' : 'text-white')+'"></i></button><div class="absolute bottom-2 right-2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full p-2.5 opacity-0 group-hover:opacity-100 transition-all shadow-black/40"><i data-lucide="play" class="w-4 h-4 text-white fill-current ml-0.5"></i></div></div><h3 class="font-semibold text-sm text-white truncate px-0.5">'+es(p.name)+'</h3><p class="text-xs text-white/60 truncate mt-0.5 px-0.5">'+(p.source === 'youtube' ? 'Playlist tersimpan' : p.songs.length+' lagu')+'</p></div></div>';
        });
        ...
        if (S.hp && S.hp.length > 0) {
            S.hp.slice(0, 8).forEach(function(p, i) {
                var playlistCover = safeMediaUrl(p.cover || FI, FI);
                plHtml += '<div onclick="Album.open(&quot;'+esJs(p.id)+'&quot;, &quot;'+esJs(playlistCover)+'&quot;)" class="relative flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-2xl bg-[#20222c] border border-white/10 shadow-xl group-hover:bg-[#282b38] transition-all flex flex-col"><div class="w-full aspect-square rounded-xl overflow-hidden relative shadow-md mb-2"><img src="'+playlistCover+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=&quot;/logo.png&quot;" /><button data-saved-playlist-id="'+esJs(p.id)+'" onclick="event.stopPropagation();toggleSavedExternalPlaylist(\''+esJs(p.id)+'\',\''+esJs(p.title)+'\',\''+esJs(playlistCover)+'\',\''+esJs(p.artist||'')+'\')" class="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/65 border border-white/20 backdrop-blur-md flex items-center justify-center" title="Simpan playlist"><i data-lucide="bookmark" class="w-4 h-4 text-white"></i></button></div><h3 class="font-semibold text-sm text-white truncate px-0.5">'+es(p.title)+'</h3><p class="text-xs text-white/60 truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div></div>';
            });
        } else if (!pls.length) {
        ...
        if (g && g.children && S.ht) {
            ...
            cards.forEach(function(el, i) {
                ...
                if (isLoad) {
                    ...
                    playIconHtml = '<div class="w-7 h-7 rounded-full btn-chrome flex items-center justify-center shrink-0 ml-auto"><svg class="w-3.5 h-3.5 fill-current ml-0.5" viewBox="0 0 24 24"><polygon points="6 3 20 12 6 21 6 3"/></svg></div>';
                }

                var cardBg = isPlay ? 'bg-[#343a4e] border border-white/40 shadow-xl' : (isCur ? 'bg-[#2e3344] border border-white/30' : 'bg-[#20222c] border border-white/10 hover:bg-[#282b38]');
                el.className = 'home-grid-card ' + cardBg + ' rounded-2xl p-0 min-w-0 overflow-hidden flex items-center gap-1.5 sm:gap-2 cursor-pointer active:scale-[0.98] transition-all duration-300 group shadow-lg shadow-black/25';

                var titleEl = el.querySelector('.home-grid-title');
                if (titleEl) {
                    titleEl.className = 'home-grid-title font-semibold text-sm text-white truncate min-w-0 ' + (isCur ? 'font-black' : '');
                }
